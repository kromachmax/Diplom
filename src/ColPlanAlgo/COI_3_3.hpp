...
{
public:
    COI_3_3() noexcept = default;
    explicit COI_3_3(std::size_t n, std::size_t m, std::vector<std::vector<T>>& D, std::vector<int>& N);
...
template<typename T>
COI_3_3<T>::COI_3_3(std::size_t n, std::size_t m, std::vector<std::vector<T>>& D, std::vector<int>& N)
...
template<typename T>
void COI_3_3<T>::Update(std::size_t n, std::size_t m, std::vector<std::vector<T> > &D, std::vector<int> &N)
{
    if (D.size() != n || (n > 0 && D[0].size() != m) || N.size() != m)
    {
...
template<typename T>
template<typename F>
bool COI_3_3<T>::notNull(const std::vector<F>& v) const noexcept
{
...
template<typename T>
bool COI_3_3<T>::isUsedRows() const noexcept
{
...
template<typename T>
bool COI_3_3<T>::isUsedCols() const noexcept
{
...
template<typename T>
void COI_3_3<T>::printMatrix(const std::vector<std::vector<T>>& matrix) const
{
    std::cout << '\n';
